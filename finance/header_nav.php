 ...
 * Shared navigation component for all finance pages
 * Uses unified VLE theme (global-theme.css + finance-dashboard.css)
 */
...
// Get finance profile picture
$finance_profile_pic = null;
if (!empty($user['related_finance_id'])) {
    $pic_conn = getDbConnection();
    $pic_stmt = $pic_conn->prepare("SELECT profile_picture FROM finance_users WHERE finance_id = ?");
    $pic_stmt->bind_param("i", $user['related_finance_id']);
    $pic_stmt->execute();
    $pic_result = $pic_stmt->get_result();
    if ($pic_row = $pic_result->fetch_assoc()) {
        $finance_profile_pic = $pic_row['profile_picture'];
    }
}
...
$nav_items = [
    ...
    ['url' => 'student_finances.php', 'icon' => 'bi-people', 'title' => 'Student Accounts'],
...
<nav class="navbar navbar-expand-lg navbar-dark vle-navbar finance-nav">
    <div class="container-fluid">
        ...
        <a class="navbar-brand d-flex align-items-center" href="dashboard.php">
            ...
            <span class="fw-bold">VLE Finance</span>
        </a>
            ...
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                ...
                <li class="nav-item dropdown">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="moreDropdown">
                        <li><a class="dropdown-item" href="charge_exam_fees.php"><i class="bi bi-journal-x me-2"></i>Exam Fees (Supp/Def)</a></li>
                        <li><a class="dropdown-item" href="student_finances.php?filter=outstanding"><i class="bi bi-exclamation-triangle me-2"></i>Outstanding Balances</a></li>
                        <li><a class="dropdown-item" href="outstanding_balances.php"><i class="bi bi-exclamation-circle me-2"></i>Defaulters List</a></li>
            ...
            <div class="navbar-nav">
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle d-flex align-items-center" href="#" id="userDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        <?php if (!empty($finance_profile_pic) && file_exists('../uploads/profiles/' . $finance_profile_pic)): ?>
                            <img src="../uploads/profiles/<?= htmlspecialchars($finance_profile_pic) ?>" class="rounded-circle me-2" style="width: 35px; height: 35px; object-fit: cover; border: 2px solid white;" alt="Profile">
                        <?php else: ?>
                            <div class="rounded-circle bg-white d-flex align-items-center justify-content-center me-2" style="width: 35px; height: 35px; color: var(--vle-primary); font-weight: 700;">
                                <?= strtoupper(substr($user['display_name'] ?? 'F', 0, 1)) ?>
                            </div>
                        <?php endif; ?>
                        <span class="d-none d-lg-inline"><?= htmlspecialchars($user['display_name'] ?? 'Finance') ?></span>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-end shadow" aria-labelledby="userDropdown">
                        <li><h6 class="dropdown-header"><i class="bi bi-person-circle me-2"></i><?= htmlspecialchars($user['display_name'] ?? 'Finance') ?></h6></li>
    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="dashboard.php"><i class="bi bi-house-door"></i> Finance Dashboard</a></li>
