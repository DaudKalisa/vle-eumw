...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['approve_registration'])) {
    ...
    $stmt = $conn->prepare("SELECT r.*, i.program_type as invite_program_type 
    ...
    if (!$reg) {
        ...
        $campus = $reg['campus'] ?? 'Mzuzu Campus';
        ...
        if ($check2->get_result()->num_rows > 0) {
            ...
            if (!$dept_data) {
                ...
                try {
                    ...
                    $success = "Student approved! ID: <strong>{$student_id}</strong> | Username: <strong>{$username}</strong> | Invoice: K" . number_format($expected_total) . $email_status;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reject_registration'])) {
    ...
    if (!$reg) {
        ...
        $stmt->bind_param("isi", $user['user_id'], $admin_notes ?: $reason, $reg_id);
        if ($stmt->execute()) {
            ...
            <div class="card-body">
                <div class="info-grid mb-3">
                    ...
                    <div class="info-item">
                        ...
                        <div class="value"><?= htmlspecialchars($reg['campus'] ?? 'N/A') ?></div>
                    </div>
                    <div class="info-item">
                            ...
                            <form method="POST">
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <strong>Campus:</strong> <?= htmlspecialchars($reg['campus'] ?? 'N/A') ?>
                                    </div>
                                    <div class="mb-3">
