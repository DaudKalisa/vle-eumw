...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message'])) {
    ...
    if (!empty($recipient_id) && !empty($subject) && !empty($message)) {
        $stmt = $conn->prepare("INSERT INTO vle_messages (sender_type, sender_id, recipient_type, recipient_id, subject, message) VALUES ('student', ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss", $student_id, $recipient_type, $recipient_id, $subject, $message);
        ...
        if ($stmt->execute()) {
            ...
            // Get sender details
            $sender_query = $conn->prepare("SELECT full_name, email FROM students WHERE student_id = ?");
            $sender_query->bind_param("s", $student_id);
...
$conversation_type = isset($_GET['type']) ? $_GET['type'] : null;
...
$lecturers = [];
$result = $conn->query("
    SELECT DISTINCT l.lecturer_id, l.full_name, vc.course_name
...
// Get students from same courses
$students = [];
$result = $conn->query("
    SELECT DISTINCT s.student_id, s.full_name
    FROM vle_enrollments ve1
    JOIN vle_enrollments ve2 ON ve1.course_id = ve2.course_id
    JOIN students s ON ve2.student_id = s.student_id
    WHERE ve1.student_id = '$student_id' AND s.student_id != '$student_id'
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
        <div class="container-fluid">
            <a class="navbar-brand" href="dashboard.php">VLE System</a>
                                ...
                                <select class="form-select" name="recipient_id_lecturer" id="recipient_id_lecturer">
                                    ...
                                    <?php foreach ($lecturers as $lecturer): ?>
                                        <option value="<?php echo $lecturer['lecturer_id']; ?>">
                                            <?php echo htmlspecialchars($lecturer['full_name']) . ' (' . htmlspecialchars($lecturer['course_name']) . ')'; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        ...
                        <div class="list-group">
                            ...
                            <?php else: ?>
                                ...
                                foreach (array_merge($inbox, $sent) as $msg) {
                                    $key = $msg['sender_type'] . '_' . $msg['sender_id'] . '_' . $msg['recipient_type'] . '_' . $msg['recipient_id'];
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" id="quickReplyBtn">Quick Reply</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function loadThread(partnerType, partnerId) {
            ...
            threadMessages.forEach(msg => {
                const isSender = msg.sender_type === 'student' && msg.sender_id === studentId;
                const className = isSender ? 'sent' : 'received';
                const bgColor = isSender ? 'bg-primary text-white' : 'bg-light';
                const align = isSender ? 'ms-auto' : 'me-auto';
                const sender = isSender ? 'You' : msg.sender_name;
